...
void FG_eval::LoadParams(const std::map<std::string, double> &params)
{
    _dbg_info   = params.find("DEBUG")      != params.end() ? params.at("DEBUG")      : _dbg_info;
...
void FG_eval::operator()(ADvector& fg, const ADvector& vars)
{
    ...
    for (int i = 0; i < _mpc_steps - 1; i++) {
        ...
        cost_accel  += _w_accel  * CppAD::pow(vars[_a_start + i],      2);
    }
    ...
    for (int i = 0; i < _mpc_steps - 2; i++) {
        ...
        cost_accel_d  += _w_accel_d  * CppAD::pow(vars[_a_start + i + 1]      - vars[_a_start + i],      2);
    }
    ...
    if(_dbg_info)
    {
        ...
        cout << "cost_angvel, cost_accel: " << cost_angvel << ", " << cost_accel << endl; 
        cout << "cost_angvel_d, cost_accel_d: " << cost_angvel_d << ", " << cost_accel_d << endl; 
    }
